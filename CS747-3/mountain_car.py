...
class sarsaAgent():
    ...
    def choose_action(self, state, weights, epsilon):
        if(np.random.uniform() > epsilon):
            return np.argmax([np.dot(state, weights[i]) for i in [0,1,2]])
        else:
    ...
    def sarsa_update(self, state, action, reward, new_state, new_action, learning_rate, weights):
        q = np.dot(state, weights[action])
        q_new = np.dot(new_state, weights[new_action])
        target = reward + q_new
        return (weights + learning_rate*(target - q))
