...
if [ ! -f "T1.png" ]; then
    echo "${red}T1.png does not get generated.${reset}"
    flag=0
else
   echo "${green}T1.png generated successfully.${reset}"
fi
if [ ! -f "T2.png" ]; then
    echo "${red}T2.png does not get generated.${reset}"
    flag=0
else
   echo "${green}T2.png generated successfully.${reset}"
fi
var="$(wc -l < out1)"
if [ ! $var = '1' ]; then
